...
std::string common_chat_format_name(common_chat_format format) {
    switch (format) {
        ...
        case COMMON_CHAT_FORMAT_DEEPSEEK_R1_EXTRACT_REASONING:
            return "DeepSeek R1 (extract reasoning)";
        ...
        case COMMON_CHAT_FORMAT_HERMES_2_PRO_EXTRACT_REASONING:
            return "Hermes 2 Pro (extract reasoning)";
        case COMMON_CHAT_FORMAT_COMMAND_R7B:
            return "Command R7B";
        case COMMON_CHAT_FORMAT_COMMAND_R7B_EXTRACT_REASONING:
            return "Command R7B (extract reasoning)";
        default:
...
static bool parse_literal(std::string::const_iterator & it, const std::string::const_iterator & end,
...
static std::optional<std::smatch> parse_pattern(std::string::const_iterator &       it,
                                                const std::string::const_iterator & end, const std::regex & expected) {
    std::smatch match;
 ...
 */
static common_chat_msg parse_json_tool_calls(const std::string & input, const std::optional<std::regex> & trigger_opt,
                                             const std::regex & function_regex, const std::regex & close_regex,
    ...
    while (it != end) {
        ...
        if (parse_json(it, end, arguments)) {
            ...
            result.tool_calls.push_back(
                { name, arguments.is_string() ? arguments.get<std::string>() : arguments.dump(), /* id= */ "" });
        } else {
            if (allow_raw_python && name == "python") {
                result.tool_calls.push_back({
                    name, json({ { "code", std::string(it, end) } }
                     ).dump(), /* id= */ ""
                });
                break;
            }
...
static common_chat_msg parse_prefixed_json_tool_call_array(const std::string & input, const std::string & prefix,
...
static common_chat_params common_chat_params_init_command_r7b(const common_chat_template &    tmpl,
    ...
    data.format =
        inputs.extract_reasoning ? COMMON_CHAT_FORMAT_COMMAND_R7B_EXTRACT_REASONING : COMMON_CHAT_FORMAT_COMMAND_R7B;
    return data;
}
...
static common_chat_params common_chat_params_init_llama_3_1_tool_calls(const common_chat_template &    tmpl,
                                                                       const struct templates_params & inputs,
...
static common_chat_msg common_chat_parse_llama_3_1(const std::string & input, bool with_builtin_tools = false) {
    ...
    static const std::regex builtin_call_regex(
...
static common_chat_params common_chat_params_init_deepseek_r1(const common_chat_template &    tmpl,
    ...
    data.prompt = prompt;
    data.format =
        inputs.extract_reasoning ? COMMON_CHAT_FORMAT_DEEPSEEK_R1_EXTRACT_REASONING : COMMON_CHAT_FORMAT_DEEPSEEK_R1;
    return data;
}

static common_chat_msg handle_think_tag_prelude(
    const std::string & input, bool extract_reasoning,
    const std::function<common_chat_msg(const std::string &)> & rest_parser) {
    std::smatch             match;
...
static common_chat_msg common_chat_parse_deepseek_r1(const std::string & input, bool extract_reasoning) {
    return handle_think_tag_prelude(input, extract_reasoning, [](const std::string & input) {
        ...
        static const std::regex close_regex("```[\\s\\r\\n]*<｜tool▁call▁end｜>");
        static const std::regex tool_calls_regex(
            "[\\s\\r\\n]*(?:<｜tool▁calls▁begin｜>|<｜tool_calls_begin｜>|<｜tool calls "
            "begin｜>|<｜tool\\\\_calls\\\\_begin｜>)([\\s\\S\\r\\n]*?)<｜tool▁calls▁end｜>");
    ...
    if (inputs.tools.is_array() && !inputs.tools.empty()) {
        ...
        data.grammar      = build_grammar([&](const common_grammar_builder & builder) {
            ...
            foreach_function(inputs.tools, [&](const json & tool) {
                ...
                first_tool_rules.push_back(builder.add_rule(
                    name + "-call", "( \"assistant<|end_header_id|>\\n\" )? \"" + name + "\\n\" " + args_rule));
                subsequent_tool_rules.push_back(
                    builder.add_rule(name + "-call2", "\">>>" + name + "\\n\" " + args_rule));
                data.grammar_triggers.push_back({
                    COMMON_GRAMMAR_TRIGGER_TYPE_PATTERN_START,
                    regex_escape(name + "\n"),
                });
            ...
            auto first_rule = first_tool_rules.empty() ?
                                       "" :
...
static common_chat_params common_chat_params_init_functionary_v3_1_llama_3_1(const common_chat_template &    tmpl,
    ...
    data.grammar      = build_grammar([&](const common_grammar_builder & builder) {
        ...
        foreach_function(inputs.tools, [&](const json & tool) {
            ...
            tool_rules.push_back(builder.add_rule(name + "-call", "\"<function=" + name + ">\" " +
                                                                           builder.add_schema(name + "-args", parameters) +
                                                                           " \"</function>\" space"));
        });
        if (has_raw_python) {
...
static common_chat_params common_chat_params_init_hermes_2_pro(const common_chat_template &    tmpl,
    ...
    data.prompt =
        apply(tmpl, inputs.messages, inputs.tools.empty() ? json() : inputs.tools, inputs.add_generation_prompt);
    data.format =
        inputs.extract_reasoning ? COMMON_CHAT_FORMAT_HERMES_2_PRO_EXTRACT_REASONING : COMMON_CHAT_FORMAT_HERMES_2_PRO;
    return data;
}
            ...
            while (it != end) {
                if (std::regex_search(it, end, match, open_regex)) {
                    ...
                    if (match[3].matched) {
                        ...
                        json tool_call;
                        if (parse_json(json_it, end, tool_call) && tool_call.contains("name") &&
...
static common_chat_params common_chat_templates_apply_jinja(const struct common_chat_templates *        tmpls,
    ...
    if (src.find("<tool_call>") != std::string::npos && params.json_schema.is_null() && params.tools.is_array() &&
