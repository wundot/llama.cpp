...
static bool common_download_file_single(const std::string & url, const std::string & path,
...
#    endif
    ...
    if (should_download) {
        ...
        LOG_INF("%s: trying to download model from %s to %s (server_etag:%s, server_last_modified:%s)...\n", __func__,
                llama_download_hide_password_in_url(url).c_str(), path.c_str(), headers.etag.c_str(),
...
static bool common_download_file_multiple(const std::vector<std::pair<std::string, std::string>> & urls,
    ...
    for (const auto & item : urls) {
        futures_download.push_back(std::async(
            std::launch::async,
            [bearer_token](const std::pair<std::string, std::string> & it) -> bool {
...
static bool common_download_model(const common_params_model & model, const std::string & bearer_token) {
 ...
 */
static struct common_hf_file_res common_get_hf_file(const std::string & hf_repo_with_tag,
                                                    const std::string & bearer_token) {
    ...
    if (res_code == 200 || res_code == 304) {
        ...
        throw std::runtime_error(
            "error: model is private or does not exist; if you are accessing a gated model, please provide a valid HF "
            "token");
    } else {
        throw std::runtime_error(
            string_format("error from HF API, response code: %ld, data: %s", res_code, res_str.c_str()));
    }
...
static bool common_download_file_multiple(const std::vector<std::pair<std::string, std::string>> &,
                                          const std::string &) {
    LOG_ERR("error: built without CURL, cannot download model from the internet\n");
    return false;
}

static bool common_download_model(const common_params_model &, const std::string &) {
    LOG_ERR("error: built without CURL, cannot download model from the internet\n");
    return false;
}
...
common_params_context common_params_parser_init(common_params & params, llama_example ex,
                                                void (*print_usage)(int, char **)) {
    ...
    add_opt(
        common_arg({ "-co", "--color" },
                   string_format("colorise output to distinguish prompt and user input from generations (default: %s)",
                                 params.use_color ? "true" : "false"),
                   [](common_params & params) { params.use_color = true; })
            .set_examples(
                { LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_INFILL, LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_LOOKUP }));
    add_opt(common_arg(
                { "-t", "--threads" }, "N",
    ...
    add_opt(
        common_arg({ "-fa", "--flash-attn" },
                   string_format("enable Flash Attention (default: %s)", params.flash_attn ? "enabled" : "disabled"),
                   [](common_params & params) { params.flash_attn = true; })
    ...
    add_opt(common_arg({ "--in-suffix" }, "STRING", "string to suffix after user inputs with (default: empty)",
                ...
                .set_examples({ LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_INFILL }));
    add_opt(common_arg({ "--no-warmup" }, "skip warming up the model with an empty run", [](common_params & params) {
                params.warmup = false;
    ...
    add_opt(common_arg({ "--top-k" }, "N",
                       ...
                       [](common_params & params, int value) { params.sampling.top_k = value; })
                .set_sparam());
    add_opt(
        common_arg({ "--top-p" }, "N",
                   string_format("top-p sampling (default: %.1f, 1.0 = disabled)", (double) params.sampling.top_p),
                   [](common_params & params, const std::string & value) { params.sampling.top_p = std::stof(value); })
            .set_sparam());
    add_opt(
        common_arg({ "--min-p" }, "N",
                   string_format("min-p sampling (default: %.1f, 0.0 = disabled)", (double) params.sampling.min_p),
                   [](common_params & params, const std::string & value) { params.sampling.min_p = std::stof(value); })
            .set_sparam());
    add_opt(
        common_arg(
            { "--top-nsigma" }, "N",
    ...
    add_opt(
        common_arg(
            ...
            [](common_params & params, const std::string & value) { params.sampling.xtc_threshold = std::stof(value); })
            .set_sparam());
    add_opt(
    ...
    add_opt(common_arg({ "--mirostat" }, "N",
                       ...
                       [](common_params & params, int value) { params.sampling.mirostat = value; })
                .set_sparam());
    ...
    add_opt(common_arg({ "-gaw", "--grp-attn-w" }, "N",
                ...
                .set_examples({ LLAMA_EXAMPLE_MAIN }));
    add_opt(common_arg({ "-dkvc", "--dump-kv-cache" }, "verbose print of the KV cache",
                       [](common_params & params) { params.dump_kv_cache = true; }));
    ...
    add_opt(common_arg({ "--perplexity", "--all-logits" },
                       string_format("return logits for all tokens in the batch (default: %s)",
                                     params.logits_all ? "true" : "false"),
                       [](common_params & params) { params.logits_all = true; })
    ...
    add_opt(common_arg({ "--ppl-output-type" }, "<0|1>",
                ...
                .set_examples({ LLAMA_EXAMPLE_PERPLEXITY }));
    add_opt(
        common_arg({ "-dt", "--defrag-thold" }, "N",
                   string_format("KV cache defragmentation threshold (default: %.1f, < 0 - disabled)",
                                 (double) params.defrag_thold),
                   [](common_params & params, const std::string & value) { params.defrag_thold = std::stof(value); })
    ...
    add_opt(common_arg({ "--mmproj" }, "FILE", "path to a multimodal projector file. see tools/llava/README.md",
                       [](common_params & params, const std::string & value) { params.mmproj.path = value; })
                .set_examples(mmproj_examples));
    add_opt(common_arg({ "--mmproj-url" }, "URL", "URL to a multimodal projector file. see tools/llava/README.md",
    ...
    add_opt(common_arg({ "--no-mmproj" }, "explicitly disable multimodal projector, useful when using -hf",
                ...
                .set_examples(mmproj_examples));
    add_opt(
        common_arg({ "--no-mmproj-offload" }, "do not offload multimodal projector to GPU", [](common_params & params) {
            params.mmproj_use_gpu = false;
        }).set_examples(mmproj_examples));
    add_opt(common_arg({ "--image" }, "FILE",
                       ...
                       "override tensor buffer type", [](common_params & params, const std::string & value) {
                           ...
                           for (const auto & override : string_split<std::string>(value, ',')) {
                               ...
                               params.tensor_buft_overrides.push_back(
                                   { strdup(tensor_name.c_str()), buft_list.at(buffer_type) });
                           }
                       }));
    ...
    add_opt(common_arg({ "-m", "--model" }, "FNAME",
                       ex == LLAMA_EXAMPLE_EXPORT_LORA ?
                           std::string("model path from which to load base model") :
                           string_format("model path (default: `models/$filename` with filename from `--hf-file` "
    ...
    add_opt(common_arg({ "--reranking", "--rerank" },
                       ...
                       [](common_params & params) { params.reranking = true; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
    ...
    add_opt(common_arg({ "--slot-save-path" }, "PATH", "path to save slot kv cache (default: disabled)",
                ...
                .set_examples({ LLAMA_EXAMPLE_SERVER }));
    add_opt(common_arg({ "--jinja" }, "use jinja template for chat (default: disabled)",
    ...
    add_opt(common_arg({ "--reasoning-format" }, "FORMAT",
                       ...
                       "controls whether thought tags are extracted from the response, and in which format they're "
                       "returned. 'none' leaves thoughts unparsed in `message.content`, 'deepseek' puts them in "
                       "`message.reasoning_content` (for DeepSeek R1 & Command R7B only).\n"
    ...
    add_opt(
        common_arg({ "--chat-template" }, "JINJA_TEMPLATE",
            ...
            .set_examples({ LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_LLAVA })
    ...
    add_opt(common_arg({ "--log-colors" }, "Enable colored logging", [](common_params &) {
    ...
    add_opt(common_arg({ "-lv", "--verbosity", "--log-verbosity" }, "N",
    ...
    add_opt(
        common_arg({ "--embd-bge-small-en-default" },
    ...
    add_opt(common_arg({ "--embd-e5-small-en-default" },
    ...
    add_opt(common_arg({ "--embd-gte-small-default" },
